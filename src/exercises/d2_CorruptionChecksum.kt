...
/**
 * Solution of the advent of code 2017 [https://adventofcode.com/2017/day/2]
 */
fun main(args: Array<String>) {
    println(corruptionChecksum(input))
}

fun corruptionChecksum(text: String): String {
    ...
    for (line in lines) {
        ...
        val matcher: Matcher = p.matcher(line)

        val numberList = mutableListOf<Long>()

        while (matcher.find()){
            numberList.add(matcher.group().toLong())
        }
        var biggest: Long = numberList[0]
        var smallest: Long = numberList[0]
        for (index in numberList.indices) {
            if (numberList[index] > biggest) biggest = numberList[index]
            if (numberList[index] < smallest) smallest = numberList[index]
        }
